...
class MetaCorgiSnacks
  def initialize(snack_box, box_id)
    @snack_box = snack_box
    @box_id = box_id
    create_snack_methods
  end

  # def method_missing(name, *args)
  #   info = "get_#{name.to_s}_info"
  #   tastiness = "get_#{name.to_s}_tastiness"
  #   methods = @snack_box.methods.map(& :to_s)

  #   if methods.include?(info) && methods.include?(tastiness)
  #     info = @snack_box.send(info.to_sym, @box_id)
  #     tastiness = @snack_box.send(tastiness.to_sym, @box_id)
